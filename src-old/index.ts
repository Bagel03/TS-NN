...
const render = () => {
    const accuracy = NN.train(data, (learnRate *= 1 - learnRateDecay));

    for (let y = 0; y < numRects; y++) {
        for (let x = 0; x < numRects; x++) {
            const [t] = NN.feedForward([x / numRects, y / numRects]);
    ...
    const newCost = NN.totalCost(data);
    if (newCost < lastCost) {
    ...
    accuracySpan.innerHTML = accuracy.toFixed(4);
